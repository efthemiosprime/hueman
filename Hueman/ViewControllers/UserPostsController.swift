//
//  UserPostsControllerViewController.swift
//  Hueman
//
//  Created by Efthemios Suyat on 5/21/17.
...
class UserPostsController: UITableViewController, UIPopoverPresentationControllerDelegate, FilterControllerDelegate {
    ...
    override func viewDidLoad() {
        ...
        backItem = UIBarButtonItem(image: UIImage(named: "back-bar-item"), style: .Plain, target: self, action: #selector(UserPostsController.backAction))
    ...
    override func viewWillAppear(animated: Bool) {
        ...
        if let unwrappedUserName = userName {
            ...
            let fullNameArr = unwrappedUserName.characters.split{$0 == " "}.map(String.init)

            self.navigationController?.navigationBar.topItem!.title = ("\(fullNameArr[0])'s posts")
        ...
        if let userUID = userUid {
            huesFeedModel.fetchUserFeeds(userUID,
...
extension UserPostsController: PopoverDelegate, MFMailComposeViewControllerDelegate {
...
extension UserPostsController: UISearchBarDelegate {
